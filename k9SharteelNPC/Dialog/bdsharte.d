...
SAY ~Crimes? There were no crimes. These dogs chained me here because I am the daughter of Angelo. That is all.~
...
++ ~Nothing really. Just wanted to see who you were.~ DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",0) ~ EXIT
...
++ ~I'll be going now.~ DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",0) ~ EXIT
END

IF ~~ THEN BEGIN 11

SAY ~As much as it pains me to think of myself working alongside these morons it still beats the smell of this place. Fine, I'll join your little group.~ IF ~~ THEN DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",2) SetGlobal("k9SharTeelPrison","GLOBAL",1) SetGlobal("SharTeelJoined","GLOBAL",1) JoinParty()~ EXIT
END
...
++ ~Nevermind.~ DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",0) ~ EXIT
END
...
+~BeenInParty("Sharteel")~+ ~Wait, is that you Shar-Teel? Last time we spoke it seemed the dungeons of the Fist would be the last place I'd find you. What changed?~ GOTO 3
++ ~Hmm, you seem familiar. Have we met before?~ GOTO 7
++ ~You are the only prisoner here that has not one but two guards. The Fist would not assign so many with so few resources at hand. Why is this?~ GOTO 9
++ ~I think a few more days with the Fist might change your attitude.~ DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",0) ~ EXIT
END
...
++ ~Hmm, you seem familiar. Have we met before?~ GOTO 7
++ ~You are the only prisoner here that has not one but two guards. The Fist would not assign so many with so few resources at hand. What do you know of Caelar?~ GOTO 9
++ ~I think a few more days with the Fist might change your attitude.~ DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",0) ~ EXIT
END

IF ~~ THEN BEGIN 22

SAY ~You're such a funny man! That's why I'm gonna kill you last.~ [SHART33] IF ~~ THEN DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",0)~ EXIT
END

IF ~~ THEN BEGIN 23

SAY ~Enough of your questions. How would you like my sword tickling your innards?~ [SHART34] IF ~~ THEN DO ~SetGlobal("k9Sharteel_Talk","GLOBAL",0)~ EXIT
END
